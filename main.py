...
def login():
    ...
    if request.method == 'POST':
        ...
        if session['code_text'] != _text:
            ...
            return render_template('login.html', error=error, username=_username)
...
def register():
    ...
    if request.method == 'POST':
        ...
        if password != pwdrepeat:
            ...
            error = '验证码错误，请重新输入'
            session.pop('code_text', None)
            return render_template('register.html', error=error, username=username, password=password, pwdrepeat=pwdrepeat, email=email)
        else:
